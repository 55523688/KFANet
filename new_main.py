...
def main(rank, world_size, config):
    ...
    if config.MODEL.BODY_PART == 'abdomen':
        weights = torch.load(f'{prefix}//Fetal-RL/data_preparation/data_biometry/ete_model/biometry_scaled_ps/class_data_split/abdomen_class_weight.pt').cuda()
    elif config.MODEL.BODY_PART == 'head':
        weights = torch.load(f'{prefix}//Fetal-RL/data_preparation/data_biometry/ete_model/biometry_scaled_ps/class_data_split/head_class_weight.pt').cuda()
    elif config.MODEL.BODY_PART == 'femur':
        weights = torch.load(f'{prefix}//Fetal-RL/data_preparation/data_biometry/ete_model/biometry_scaled_ps/class_data_split/femur_class_weight.pt').cuda()
    elif config.MODEL.BODY_PART == 'all':
        weights = torch.load(f'{prefix}//Fetal-RL/data_preparation/data_biometry/ete_model/biometry_scaled_ps/class_data/weights_all.pt').cuda()
    ...
    for idx, (images, Class, measure, ps, frames_n, measure_scaled, index, days_normalized, frame_loc, measure_normalized, org_seq_lens) in enumerate(data_loader):
        ...
        if config.MODEL.TASK_TYPE == 'cls':
            ...
            if config.DATA.IMG_SCALING:
                scaler = joblib.load(f'{prefix}//Fetal-RL/data_preparation/data_biometry/ete_model/biometry_scaled_ps/normalizer_measure_img_scaling')
                predicted_measure = scaler.inverse_transform(outputs.cpu().numpy())
            else:
                if config.MODEL.BODY_PART == 'head':
                    scaler = joblib.load(f'{prefix}//Fetal-RL/data_preparation/data_biometry/ete_model/biometry_scaled_ps/head/head_scaler')
                elif config.MODEL.BODY_PART == 'abdomen':
                    scaler = joblib.load(f'{prefix}//Fetal-RL/data_preparation/data_biometry/ete_model/biometry_scaled_ps/abdomen/abdomen_scaler')
                elif config.MODEL.BODY_PART == 'femur':
                    scaler = joblib.load(f'{prefix}//Fetal-RL/data_preparation/data_biometry/ete_model/biometry_scaled_ps/femur/femur_scaler')
                elif config.MODEL.BODY_PART == 'all':
                    scaler = joblib.load(f'{prefix}//Fetal-RL/data_preparation/data_biometry/ete_model/biometry_scaled_ps/all/all_scaler')
